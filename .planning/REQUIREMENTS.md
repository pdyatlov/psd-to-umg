...
- [ ] **TEXT-02**: Bold and italic weight/style applied via TypefaceFontName
...
- [ ] **LAYOUT-01**: _9s / _9slice suffix → Box draw mode with configurable margins
- [ ] **LAYOUT-02**: Margin syntax in layer name: MyButton_9s[16,16,16,16]
...
- [ ] **EDITOR-01**: Plugin settings in Project Settings → Plugins → PSD2UMG (output dirs, font map, source DPI, effect handling, anchor auto-assign, CommonUI mode)
- [ ] **EDITOR-02**: Import preview dialog shows layer tree with checkboxes and widget type badges before creating assets
- [ ] **EDITOR-03**: User can toggle individual layers on/off before import
...
- [ ] **TEST-01**: Unit tests (no UE dependency): FPsdParser, FLayerMappingRegistry, FAnchorCalculator, DPI conversion
- [ ] **TEST-02**: Integration tests (FAutomationTestBase): full PSD→WBP pipeline, reimport
...
- [ ] **TEST-04**: GitHub Actions CI: compile on Win64 + run unit tests on push

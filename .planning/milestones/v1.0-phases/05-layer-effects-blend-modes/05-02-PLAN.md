...
// For non-image layers (group/text) with complex effects, RGBAPixels will be
// empty and this guard prevents incorrect flattening — a warning is logged.
...
```
  ...
  <done>All five effects applied in generator: opacity sets RenderOpacity, hidden layers get Collapsed, color overlay tints UImage brush, drop shadow creates offset sibling, complex effects flatten to image when setting enabled. Non-image layers with complex effects log warning when RGBAPixels empty.</done>
</task>
...
**Change 3 — Add new test: "should apply color overlay tint to UImage brush" (FX-03, per BLOCKER 3):**
...
**Change 4 — Add new test: "should flatten layer with complex effects when setting enabled":**
...
**Change 5 — Add new test: "should create shadow sibling for drop shadow layers" (FX-04):**
...
```
  ...
  <verify>
    <automated>grep -c "should create invisible layers as Collapsed" Source/PSD2UMG/Tests/FWidgetBlueprintGenSpec.cpp && grep -c "should apply opacity" Source/PSD2UMG/Tests/FWidgetBlueprintGenSpec.cpp && grep -c "should apply color overlay tint" Source/PSD2UMG/Tests/FWidgetBlueprintGenSpec.cpp && grep -c "should flatten layer" Source/PSD2UMG/Tests/FWidgetBlueprintGenSpec.cpp && grep -c "should create shadow sibling for drop shadow layers" Source/PSD2UMG/Tests/FWidgetBlueprintGenSpec.cpp</automated>
  </verify>
  <done>Existing invisible-layer test updated to assert Collapsed instead of skip; new tests for opacity, color overlay tint (FX-03), flatten, and drop shadow sibling (FX-04); all tests use concrete FPsdDocument fixtures</done>
</task>
...
- New opacity, color overlay, flatten, and drop shadow sibling tests present
...
- Non-image complex-effect layers with no RGBAPixels log warning and skip flatten
- Tests verify opacity, collapsed visibility, color overlay tint, drop shadow sibling, and flatten behaviors
</success_criteria>

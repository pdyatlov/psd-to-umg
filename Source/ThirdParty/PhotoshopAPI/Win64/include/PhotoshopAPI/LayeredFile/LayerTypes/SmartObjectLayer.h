...
struct SmartObjectLayer final: public Layer<T>, public ImageDataMixin<T>
{
		...
		: Layer<T>(layerRecord, channelImageData, header)
	{
		...
		// Cache the PSD's stored rasterized preview (Photoshop's pre-rendered composite for
		// this layer). Layer<T> only extracts the mask channel; the RGBA channels remain in
		// channelImageData. By caching them here, evaluate_channel() short-circuits to return
		// this stored data instead of throwing when the linked file (.ai etc.) is unavailable.
		for (int i = 0; i < layerRecord.m_ChannelCount; ++i)
		{
			const auto& channel_info = layerRecord.m_ChannelInformation[i];
			if (channel_info.m_ChannelID.id == Enum::ChannelID::UserSuppliedLayerMask) continue;
			auto channelPtr = channelImageData.extract_image_ptr(channel_info.m_ChannelID);
			if (!channelPtr) continue;
			ImageDataMixin<T>::m_ImageData[channel_info.m_ChannelID] = std::move(channelPtr);
